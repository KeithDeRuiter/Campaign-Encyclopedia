...
public class CampaignTree {
    ...
    private void expandDomainNodes(boolean expand) {
        DefaultMutableTreeNode rootNode = (DefaultMutableTreeNode) m_entityTree.getModel().getRoot();
        
        //Iterate over all children of the root, and expand if they are domains (should always be for now, more of a protection)
        for (TreeNode node : JTreeUtil.children(rootNode)) {
    ...
    private DefaultMutableTreeNode matchAndBuildNode(DefaultMutableTreeNode oldNode) {
        ...
        for (TreeNode childOldNode : JTreeUtil.children(oldNode)) {
            DefaultMutableTreeNode mutableChild = (DefaultMutableTreeNode) childOldNode;
            DefaultMutableTreeNode newMatchedChildNode = matchAndBuildNode(mutableChild);
